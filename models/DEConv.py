...
class Conv2d_cd(nn.Module):
    ...
    def get_weight(self):
        ...
        conv_weight_cd = torch.zeros(
            conv_shape[0], conv_shape[1], 3 * 3, device=conv_weight.device
        )
...
class Conv2d_rd(nn.Module):
    ...
    def forward(self, x):
        if math.fabs(self.theta - 0.0) < 1e-8:
            ...
            conv_weight = self.conv.weight
            conv_shape = conv_weight.shape
            conv_weight_rd = torch.zeros(
                conv_shape[0], conv_shape[1], 5 * 5, device=conv_weight.device
            )
            conv_weight = Rearrange("c_in c_out k1 k2 -> c_in c_out (k1 k2)")(
                conv_weight
            )
            conv_weight_rd[:, :, [0, 2, 4, 10, 14, 20, 22, 24]] = conv_weight[:, :, 1:]
            conv_weight_rd[:, :, [6, 7, 8, 11, 13, 16, 17, 18]] = (
                -conv_weight[:, :, 1:] * self.theta
            )
            conv_weight_rd[:, :, 12] = conv_weight[:, :, 0] * (1 - self.theta)
            conv_weight_rd = conv_weight_rd.view(conv_shape[0], conv_shape[1], 5, 5)
            out_diff = nn.functional.conv2d(
                input=x,
                weight=conv_weight_rd,
                bias=self.conv.bias,
...
class Conv2d_hd(nn.Module):
    ...
    def get_weight(self):
        ...
        conv_weight_hd = Rearrange(
            "c_in c_out (k1 k2) -> c_in c_out k1 k2", k1=conv_shape[2], k2=conv_shape[2]
        )(conv_weight_hd)
...
class DEConv(nn.Module):
    ...
    def forward(self, x):
        ...
        res = nn.functional.conv2d(
            input=x, weight=w, bias=b, stride=1, padding=1, groups=1
        )
        return res
